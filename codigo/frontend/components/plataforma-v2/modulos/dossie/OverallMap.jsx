...
/* Dossie 02 - Overall Map (radar hexagonal + breakdown).
 * Adaptado de designer/section-overall.jsx. */

import { useState } from "react";
import { SectionShell } from "../../Primitives";
import { DIMENSIONS, STATS, STAT_BREAKDOWN } from "../../data";

function resolveStats(profile) {
  if (profile?.overallStats) return profile.overallStats;
  return STATS[profile?.sparse ? "marcal" : "wagner"];
}
...
function HexRadar({ values, size = 260 }) {
  ...
  return (
    <svg viewBox={`0 0 ${size} ${size}`} style={{ width: "100%", maxWidth: size, height: "auto" }}>
      ...
      {DIMENSIONS.map((d, i) => {
        const [x, y] = ringPoints(1)[i];
        return <line key={d.key} x1={cx} y1={cy} x2={x} y2={y} stroke="var(--rule)" strokeWidth="0.75" />;
      })}
      <defs>
      ...
      {valuePoints.map(([x, y], i) => (
        <circle key={i} cx={x} cy={y} r="3.5" fill="var(--bg-card)" stroke="#60a5fa" strokeWidth="1.5" />
      ))}
      {DIMENSIONS.map((d, i) => {
        const a = angle(i);
...
function StatRow({ dim, value, isActive, onClick }) {
  ...
  const c = scoreColor(hasValue ? value : 0);
  return (
    <button
      type="button"
      onClick={onClick}
    ...
    >
      ...
      >
        ...
        <div className="text-[11px] font-black tnum leading-none mt-0.5" style={{ color: c.fg }}>{hasValue ? value : "-"}</div>
      </div>
      <div className="flex-1 min-w-0">
        <div className="flex items-center justify-between mb-1">
          <div className="text-[12px] font-semibold t-fg-strong">{dim.label}</div>
          <div className="text-[10px] font-mono t-fg-dim">
            <span className="font-bold" style={{ color: c.fg }}>{hasValue ? c.grade : "-"}</span>
            <span className="t-fg-ghost"> · </span>
            <span className="tnum">{hasValue ? value : "-"}/100</span>
          </div>
        </div>
        <div className="h-1.5 rounded-full overflow-hidden" style={{ background: "var(--rule)" }}>
          <div
            className="h-full rounded-full transition-all duration-500"
            style={{ width: `${hasValue ? value : 0}%`, background: `linear-gradient(90deg, ${c.fg}88, ${c.fg})` }}
          />
        </div>
      </div>
    </button>
    ...
    <div className="h-full rounded-lg p-5" style={{ background: "var(--bg-card-2)", border: "1px solid var(--rule)" }}>
      ...
      <div className="mt-5 pt-4 grid grid-cols-3 gap-3" style={{ borderTop: "1px solid var(--rule)" }}>
        <div>
          ...
          <div className="text-[15px] font-bold tnum t-fg-strong mt-0.5">
            Top {value >= 85 ? "10" : value >= 70 ? "25" : value >= 50 ? "50" : "70"}%
          </div>
        </div>
        <div>
          <div className="text-[9px] font-mono tracking-[0.15em] uppercase t-fg-dim">Variação</div>
          <div
            className="text-[15px] font-bold tnum mt-0.5"
            style={{ color: value >= 60 ? "#22c55e" : "#ef4444" }}
          >
            {value >= 60 ? "▲ +4" : "▼ -7"}
          </div>
        </div>
...
export function OverallMap({ profile }) {
  const values = resolveStats(profile);
  const [selKey, setSelKey] = useState("BSE");
  const dim = DIMENSIONS.find((d) => d.key === selKey);
  const disponiveis = Object.values(values || {}).filter((v) => v !== null && v !== undefined);
  const overall = disponiveis.length
    ? Math.round(disponiveis.reduce((a, b) => a + b, 0) / disponiveis.length)
    : 0;
  ...
  return (
    <SectionShell
      id="sec-overall"
      label="02 Overall"
      title="Mapa do Overall"
      sub="stat breakdown"
      kicker={
        <>
          <span>Média ponderada: <span className="t-fg-strong font-bold tnum">{overall}</span></span>
    ...
    >
      <div className="grid gap-6" style={{ gridTemplateColumns: "320px 280px 1fr" }}>
        <div className="flex flex-col">
          ...
          <div
            className="mt-2 pt-4 flex items-center justify-between text-[10px] font-mono t-fg-dim"
            style={{ borderTop: "1px solid var(--rule)" }}
          >
            <div className="flex items-center gap-1.5">
              <span className="w-2 h-2 rounded-sm" style={{ background: "#3b82f6" }} />Candidato
            </div>
            <div className="flex items-center gap-1.5">
              <span
                className="w-2 h-2 rounded-sm"
                style={{ background: "var(--rule-strong)", boxShadow: "inset 0 0 0 1px var(--fg-ghost)" }}
              />
              Grade (0-100)
            </div>
          </div>
