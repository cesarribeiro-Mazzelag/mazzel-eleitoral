...
/* Dossie 08 - Emendas parlamentares.
 * Adaptado de designer/section-emendas.jsx. */

import { Icon } from "../../Icon";
import { SectionShell, EmptyState, KPI } from "../../Primitives";

const STATUS_CLS = {
  "Pago":        "chip-green",
  "Em execução": "chip-amber",
  "Cancelado":   "chip-red",
};

function Donut({ data, size = 180, executadoLabel = "R$ 42M" }) {
...
export function Emendas({ profile }) {
    ...
    return (
      <SectionShell id="sec-emendas" label="08 Emendas" title="Emendas Parlamentares" sub="execução e destino">
        <EmptyState
          ...
          title="Módulo Emendas em construção"
          text="Integração com Portal da Transparência (CGU) e APIs Câmara/Senado prevista no roadmap. Quando ativada, esta seção mostrará valor apresentado, aprovado, executado e taxa de execução por parlamentar."
        />
      </SectionShell>
    );
  }
  // Guard contra anual vazio (evita Math.max([]) = -Infinity)
  const maxYear = (E.anual && E.anual.length > 0) ? Math.max(...E.anual.map((a) => a.v)) : 0;
  ...
  return (
    ...
    >
      <div className="grid grid-cols-4 gap-3">
        ...
        >
          <div className="text-[10px] font-mono tracking-[0.15em] uppercase" style={{ color: "var(--ok)" }}>
            Taxa de execução
          </div>
          <div className="flex items-baseline justify-between mt-1">
            <div className="text-[30px] font-black font-display tnum" style={{ color: "var(--ok)" }}>
              {E.taxa}%
            </div>
        ...
        <div className="rounded-xl overflow-hidden" style={{ border: "1px solid var(--rule)" }}>
          {E.top.map((t, i) => (
            ...
            >
              ...
              <span
                className={`chip ${STATUS_CLS[t.st] || "chip-muted"}`}
      ...
      >
        <div className="text-[11px] t-fg-muted">
          <span className="font-bold t-fg">Contexto:</span> Taxa de execução alta indica articulação real com o Executivo para liberação orçamentária.
        </div>
      </div>
    </SectionShell>
  );
