...
/* Dossie 07 - Financeiro de campanha.
 * Adaptado de designer/section-financeiro.jsx. */
...
export function Financeiro({ profile }) {
    ...
    return (
      <SectionShell id="sec-financeiro" label="07 Financeiro" title="Financeiro de Campanha" sub="arrecadação · gasto · CPV">
        <EmptyState
          ...
          title="TSE ainda não publicou dados do ciclo"
          text="A prestação de contas do ciclo eleitoral atual ainda está em análise no TSE."
        />
      </SectionShell>
    );
  }
  // Guard contra fontes vazias (Math.max([]) = -Infinity quebra renderizacao das barras)
  const max = (F.fontes && F.fontes.length > 0) ? Math.max(...F.fontes.map((f) => f.v)) : 0;
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="text-right">
            <div className="text-[11px] t-fg-muted">
              {F.cpv.label}: <span className="font-mono t-fg tnum">{F.cpv.medianaCargo}</span>
            </div>
            <div
