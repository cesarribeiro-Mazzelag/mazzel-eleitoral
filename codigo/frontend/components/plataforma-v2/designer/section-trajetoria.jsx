/* Section 04 — Trajetória (timeline horizontal) */

function TrajetoriaTimeline({ profile }) {
  ...
  return (
    <SectionShell id="sec-trajetoria" label="04 Trajetória" title="Trajetória Política" sub="linha do tempo"
      kicker={<span>{events.length} eventos</span>}>
      <div className="relative">
        {/* arrows */}
        <button onClick={()=>scroll(-360)}
          className="absolute left-0 top-1/2 -translate-y-1/2 z-10 w-8 h-8 rounded-full flex items-center justify-center"
          style={{ background:"var(--bg-card)", border:"1px solid var(--rule-strong)", color:"var(--fg-muted)" }}>
          <ArrowLIcon size={14}/>
        </button>
        <button onClick={()=>scroll(360)}
        ...
        <div ref={scrollRef} className="timeline-scroll overflow-x-auto px-10 pb-2">
          <div className="relative" style={{ minWidth: events.length * 220 }}>
            {/* horizontal line */}
            <div className="absolute left-0 right-0 h-px" style={{ top: 32, background:"var(--rule-strong)" }}/>
            {/* events */}
            <div className="flex items-start" style={{ gap: 0 }}>
              {events.map((ev, i) => {
                const prev = events[i-1];
                const gap = prev ? (parseInt(ev.ano) - parseInt(prev.ano)) : 0;
                const [stLabel, stColor] = statusLabel(ev.status);
                return (
                  <div key={i} className="flex-shrink-0 relative flex flex-col items-center" style={{ width: 220 }}>
                    {/* dotted if gap */}
                    {prev && gap > 4 && (
                      <div className="absolute" style={{ top: 30, left: "-50%", width: "50%", height: 4, borderTop: "2px dotted var(--fg-ghost)" }}/>
                    )}
                    {/* node */}
                    <div className={`w-4 h-4 rounded-full ${nodeClass(ev.status)}`} style={{ marginTop: 24 }}/>
                    {/* card */}
                    <div className="mt-5 w-[200px] rounded-lg px-3 py-3 text-center"
                      ...
                      <div className="text-[10px] font-bold uppercase tracking-[0.15em] mt-1.5" style={{ color: stColor }}>{stLabel}</div>
                      ...
                      <div className="text-[11px] t-fg-dim mt-0.5 flex items-center justify-center gap-1.5">
                        ...
                        <span className="chip chip-muted !h-[16px] !px-1.5 !text-[9px]">{ev.partido}</span>
                      </div>
        ...
        {/* legend */}
        <div className="mt-3 pt-4 flex items-center gap-5 text-[10px] font-mono t-fg-dim" style={{ borderTop:"1px solid var(--rule)" }}>
...
Object.assign(window, { TrajetoriaTimeline });
