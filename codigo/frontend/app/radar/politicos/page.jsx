...
/**
 * /radar/politicos - listagem dos politicos como cartas FIFA + filtros FUT.
 */
...
function LinhaPolitico({ politico, onAbrir }) {
  ...
  return (
    <button
      onClick={() => onAbrir(politico.candidato_id)}
      className="w-full grid grid-cols-[auto_auto_1fr_auto_auto_auto] items-center gap-3 px-3 py-2 bg-white border border-gray-200 rounded-lg hover:border-brand-300 hover:shadow-sm transition-all text-left"
    >
      <span className={`inline-flex items-center justify-center w-10 h-10 rounded-lg border ${tier.bg} font-black text-base tabular-nums`} style={{ fontFamily: "Georgia, serif" }}>
        {politico.overall ?? "-"}
      </span>
      ...
      <div className="min-w-0">
        ...
        <p className="text-[10px] text-gray-500 truncate">
          {politico.cargo} · {localidade} · {politico.ano}
        </p>
      </div>
      {politico.partido_sigla && (
        <span className="text-[10px] font-bold text-white px-1.5 py-0.5 rounded" style={{ backgroundColor: politico.partido_cor || "#94A3B8" }}>
          {politico.partido_sigla}
        </span>
      )}
      <span className="text-xs font-bold text-gray-700 tabular-nums">
        {fmt(politico.votos_melhor_turno ?? politico.votos_total)}
      </span>
...
export default function RadarPoliticosPage() {
  ...
  }, [filtros.cargo, filtros.estado_uf, filtros.ano, filtros.tier, filtros.trait,
      buscaDebounced, filtros.ordenar_por, filtros.pagina]);

  useEffect(() => {
    let cancelado = false;
    setCarregando(true);
    setErro(null);
    fetch(`${API}/radar/politicos?${queryString}`, {
  ...
  const totalPaginas = resposta
    ? Math.max(1, Math.ceil(resposta.total / resposta.por_pagina))
    : 1;

  // Contadores por tier (usando apenas a pagina atual - backend nao retorna agregado)
  const contadoresPorTier = useMemo(() => {
    if (!resposta?.items) return null;
    const c = { dourado: 0, ouro: 0, prata: 0, bronze: 0 };
    for (const p of resposta.items) {
    ...
    <AppLayout>
      <div className="max-w-7xl mx-auto space-y-4">
        ...
        <FiltrosFUT
          filtros={filtros}
          onChange={setFiltros}
          contadoresPorTier={contadoresPorTier}
          vista={vista}
          ...
          <div className="bg-white border border-gray-200 rounded-lg p-3 space-y-2">
            <div className="flex items-center justify-between">
              <p className="text-sm text-gray-700">
                ...
                {" "}político{resposta.total === 1 ? "" : "s"}
                {temFiltros ? " encontrado" : " no total"}{resposta.total === 1 ? "" : "s"}
              </p>
            </div>
        ...
        {erro && !carregando && (
          <div className="card p-6 text-center text-red-600 text-sm">
            Erro ao carregar políticos: {erro}
          </div>
        )}

        {/* Vista Grid (cartas FIFA) */}
        {!carregando && !erro && resposta?.items?.length > 0 && vista === "grid" && (
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
            {resposta.items.map((p) => (
              <CardPolitico
        ...
        {!carregando && !erro && resposta?.items?.length === 0 && (
          <div className="card p-10 text-center text-gray-500 text-sm">
            Nenhum político encontrado com os filtros atuais.
          </div>
        )}
        ...
        {resposta && totalPaginas > 1 && (
          <div className="flex items-center justify-between pt-2">
            <p className="text-xs text-gray-500">
              Página {resposta.pagina} de {totalPaginas}
            </p>
            <div className="flex items-center gap-2">
              <button
                disabled={resposta.pagina <= 1}
              ...
              <button
                disabled={resposta.pagina >= totalPaginas}
                onClick={() => setFiltros({ ...filtros, pagina: filtros.pagina + 1 })}
                className="btn-outline text-xs flex items-center gap-1 disabled:opacity-30"
              >
                Próxima
