"use client";
/**
 * Painel Administrativo — apenas PRESIDENTE
 * Gestão de usuários, auditoria e configurações sistêmicas
 */
import { useState, useEffect } from "react";
import {
  Shield, Users, ClipboardList, ToggleLeft, ToggleRight,
...
function AbaUsuarios() {
  ...
  if (carregando) {
    return <div className="flex justify-center py-16"><div className="w-8 h-8 rounded-full border-4 border-uniao-azul border-t-transparent animate-spin" /></div>;
  }
...
function AbaAuditoria() {
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
      ...
      {logs.length === 0 && (
        <p className="text-center text-sm text-gray-400 py-10">Nenhum log de auditoria disponível.</p>
      )}
    </div>
  );
}
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-2">
            <Shield className="w-6 h-6 text-uniao-azul" />
            Administração
          </h1>
