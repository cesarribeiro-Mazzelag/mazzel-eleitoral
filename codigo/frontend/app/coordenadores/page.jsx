...
/**
 * Módulo Coordenadores — Gestão de Territórios
 *
 * Cada coordenador é responsável por ~50 municípios.
 * A diretoria configura os territórios; o mapa exibe cada um em uma cor distinta.
 *
 * Abas:
 *   Visão Geral   — lista de coordenadores com KPIs
 *   Mapa          — mapa com territórios coloridos por coordenador
 *   Gerenciar     — atribuição de municípios (PRESIDENTE/DIRETORIA)
 */

import { useEffect, useState, useMemo, useCallback, useRef } from "react";
import { AppLayout } from "@/components/layout/AppLayout";
import {
  Map, Users, Plus, Pencil, Trash2, Search, ChevronRight,
...
function ModalCoordenador({ coord, uf, onSalvar, onFechar }) {
  ...
  async function salvar() {
    ...
    if (!form.estado_uf)   { toast.error("Estado é obrigatório"); return; }
    setSalvando(true);
    try {
      const url   = coord ? `${API}/coordenadores/${coord.id}` : `${API}/coordenadores`;
      const meth  = coord ? "PUT" : "POST";
      const res   = await fetch(url, { method: meth, headers: hdr(), body: JSON.stringify(form) });
...
function PainelDetalhe({ coordId, onFechar, onEditar, onExcluir, perfil }) {
  ...
  useEffect(() => {
    if (!coordId) return;
    setCarregando(true);
    fetch(`${API}/coordenadores/${coordId}`, { headers: { Authorization: `Bearer ${tkn()}` } })
      .then(r => r.json())
      .then(setDados)
      .catch(console.error)
          ...
          <div className="min-w-0 flex-1 pr-3">
            ...
            {carregando ? (
              ...
              <>
                ...
                <p className="text-xs text-blue-300 mt-0.5">{c?.estado_uf} · {stats?.total_municipios} municípios</p>
              </>
            )}
          </div>
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          <div className="px-4 py-3">
            ...
            {dados.municipios.length === 0 ? (
              <p className="text-xs text-gray-400 py-4 text-center">Nenhum município atribuído</p>
            ) : (
      ...
      <div className="p-4">
        ...
        <div className="grid grid-cols-3 gap-2 mb-3">
          <div className="text-center">
            ...
            <p className="text-[10px] text-gray-400">municípios</p>
          </div>
          <div className="text-center">
        ...
        <div>
          <div className="flex items-center justify-between mb-1">
            <span className="text-[10px] text-gray-400">Municípios com eleito</span>
...
// ── Painel de Atribuição de Municípios ────────────────────────────────────────
...
function PainelAtribuicao({ coordId, coordNome, coordUf, corHex, onFechar, onAtualizar }) {
  ...
  useEffect(() => {
    ...
    fetch(`${API}/coordenadores/sem-coordenador/${coordUf}`, {
  ...
  async function atribuir() {
    if (selecionados.size === 0) { toast.error("Selecione ao menos um município"); return; }
    setSalvando(true);
    try {
      const res = await fetch(`${API}/coordenadores/${coordId}/municipios`, {
        method: "POST",
        headers: hdr(),
        body: JSON.stringify({ municipio_ids: [...selecionados] }),
      });
      if (!res.ok) throw new Error("Erro ao atribuir");
      toast.success(`${selecionados.size} município(s) atribuído(s)`);
      setSel(new Set());
      onAtualizar();
      onFechar();
    } catch (e) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg mx-4 flex flex-col max-h-[80vh]">
        ...
        <div className="flex items-center justify-between px-6 pt-6 pb-4 border-b border-gray-100">
          <div>
            <h2 className="text-base font-bold text-gray-900">Atribuir Municípios</h2>
        ...
        <div className="px-6 py-3 border-b border-gray-100">
          ...
          {selecionados.size > 0 && (
            <p className="text-xs text-blue-600 font-semibold mt-2">
              {selecionados.size} município(s) selecionado(s)
            </p>
          )}
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-3 py-2">
          {filtrados.length === 0 ? (
            <div className="py-8 text-center text-sm text-gray-400">
              {semCoord.length === 0
                ? "Todos os municípios já têm coordenador"
...
export default function CoordenadoresPag() {
  ...
  const carregar = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`${API}/coordenadores?${params}`, {
        headers: { Authorization: `Bearer ${tkn()}` },
      });
      const data = await res.json();
  ...
  async function excluir(coord) {
    ...
    try {
      await fetch(`${API}/coordenadores/${coord.id}`, {
        ...
        <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
          ...
          <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 bg-white flex-shrink-0">
            <div>
              ...
              <p className="text-xs text-gray-500">Gestão de territórios e performance por coordenador</p>
            </div>
          ...
          <div className="grid grid-cols-4 gap-4 px-6 py-4 border-b border-gray-100 bg-gray-50 flex-shrink-0">
            {[
              ...
              { label: "Municípios",        val: kpis.totalMuns,   icon: MapPin,   color: "text-green-700",  bg: "bg-green-50" },
          ...
          <div className="flex-1 overflow-y-auto p-6">
            {carregando ? (
              ...
              <>
                ...
                {/* Ação: atribuir municípios ao coordenador selecionado */}
                  ...
                  return c ? (
                    <div className="mt-6 flex items-center justify-between bg-blue-50 border border-blue-200 rounded-2xl px-5 py-3">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs text-blue-600">- {c.total_municipios} municípios</span>
                      </div>
                      <button
