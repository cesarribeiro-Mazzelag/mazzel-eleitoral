...
/**
 * /glossario - pagina de referencia para entender o Radar Politico.
 *
 * Acesso: item na sidebar + link "Como funciona" no header do Radar.
 * Conteudo organizado em 4 secoes: Overall e Tiers, Atributos, Insignias, Traits.
 */
...
const TIERS = [
  { v: "dourado", label: "Dourado", faixa: "85-99",   desc: "Elite. Alta performance em multiplos atributos.",                      bg: "bg-amber-50", border: "border-amber-200", text: "text-amber-800" },
  ...
  { v: "prata",   label: "Prata",   faixa: "65-74",   desc: "Medio. Pontos fortes especificos, oportunidade de crescer.",           bg: "bg-gray-50",  border: "border-gray-200",  text: "text-gray-700" },
  { v: "bronze",  label: "Bronze",  faixa: "<65",     desc: "Base, renovacao, ou trajetoria em construcao.",                        bg: "", border: "", text: "" },
];
...
const CARGOS = [
  { cargo: "PRESIDENTE",        metal: "ouro",   hierarquia: "Executivo federal - honra maxima" },
      ...
      <div className="max-w-5xl mx-auto px-6 py-8 space-y-6">
        <header className="mb-4">
          ...
          <p className="text-sm text-gray-600">
            Como ler a carta política do Radar. Em 1 minuto você entende todos os elementos.
          </p>
        </header>
        ...
        <Secao titulo="Overall e tiers" icon={Trophy}>
          <p className="text-sm text-gray-600 mb-4">
            ...
            (6 visíveis no card, 2 adicionais no dossiê). A coroa de louros em volta do número
            indica <strong>eleito atualmente</strong> - quem disputou e perdeu não tem laurel.
          </p>
            ...
            {TIERS.map(t => (
              <div key={t.v} className={`rounded-xl border ${t.border} ${t.bg} p-4`} style={t.v === "bronze" ? { background: "rgba(184,115,51,0.08)", borderColor: "rgba(184,115,51,0.25)" } : {}}>
                <div className="flex items-baseline justify-between mb-2">
                  <span className={`text-[10px] font-black tracking-widest ${t.text}`} style={t.v === "bronze" ? { color: "#b87333" } : {}}>
                    {t.label}
                  </span>
                ...
                <div className="relative w-16 h-16 mx-auto my-3">
                  ...
                  <div className="absolute inset-0 flex flex-col items-center justify-center">
                    <span className="font-black text-gray-900 tabular-nums" style={{ fontSize: 18 }}>
                      {t.faixa.includes("-") ? t.faixa.split("-")[0] : "60"}
                    </span>
          ...
          <div className="grid grid-cols-2 gap-3">
            {ATRIBUTOS.map(a => (
              <div key={a.k} className="flex items-start gap-3 p-3 rounded-lg bg-gray-50">
                <div className="w-10 h-10 rounded-lg bg-white border border-gray-200 flex items-center justify-center flex-shrink-0 font-black text-gray-700 text-xs tracking-wider">
                  {a.k}
                </div>
                <div>
        ...
        <Secao titulo="Insígnias por cargo" icon={Trophy}>
          <p className="text-sm text-gray-600 mb-4">
            Cada cargo tem uma insígnia única - medalha, escudo, coroa - seguindo hierarquia
            política. Aparece no canto superior direito do card (cargo atual) e no rodapé em
            sépia (cargos já ocupados). Cor cheia = está ocupando agora. Tracejado = disputou
            e não venceu.
          </p>
            ...
            {CARGOS.map(c => (
              <div key={c.cargo} className="flex flex-col items-center p-4 rounded-xl bg-gray-50">
                ...
                <div className="mt-3 text-center">
                  <div className="text-[10px] font-black tracking-widest text-gray-800">
                    {c.cargo}
                  </div>
        ...
        <Secao titulo="Traits políticos" icon={Sparkles}>
          <p className="text-sm text-gray-600 mb-4">
            Marcadores com sentido político real. Calculados automaticamente a partir do histórico
            de candidaturas e resultados.
          </p>
          <div className="grid grid-cols-2 gap-3">
            {TRAITS.map(t => {
              const Icon = t.icon;
