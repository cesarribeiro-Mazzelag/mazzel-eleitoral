...
| 7 | `docs/mapa_eleitoral_evolucao.md` | **Pra onde o Mapa precisa evoluir** — conceito de navegação atual aprovado (drill-down 5 níveis + foto-on-hover), 10 áreas de salto pra produção, **3 Modos pré-configurados** (Saúde Operacional / Fluxo de Dinheiro / Compliance), **bug do mapa no Dossiê** (heatmap quadriculado precisa virar mapa geográfico real). |
| 8 | `docs/modulo_emendas.md` | **Módulo NOVO** — controle financeiro do partido + auditoria + alertas. Mapa de Emendas + Dossiê da Emenda + Painel de Inconsistências (algoritmo 5 dimensões) + alertas em tempo real priorizados. RBAC restrito a alta liderança. |
| 9 | `docs/api_sgip_descoberta.md` | **Descoberta crítica** — SGIP3 do TSE expõe API pública completa (44 partidos × ~50-100k órgãos × milhões de membros com CPF+título+cargo). Fonte canônica das nominatas. Saúde da Nominata cruza SGIP × atividade da plataforma. |
| 10 | `docs/etl_sgip_spec.md` | Spec técnica detalhada do ETL SGIP (apenas leitura — implementação é pelo Claude Code). Você não precisa saber detalhes do crawler, mas é útil saber que **dados de nominata virão de fonte oficial TSE com cobertura nacional**, então você pode confiar na completude dos dados ao desenhar a UI. |
| 11 | `STATUS_DADOS_V2.md` (raiz do worktree) | Qual módulo é REAL, qual é HIBRIDO (API+fallback mock), qual é MOCK puro, qual é PLACEHOLDER. Pra você não estilizar baseado em magnitudes erradas. |
